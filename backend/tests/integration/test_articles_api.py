...
async def test_get_article_ends_with_deterministic_podcast_attribution(db_session: AsyncSession) -> None:
    """Regression test for the Live/Draft Article Workflow follow-up fix
    (requirement #4): the reviewer-facing GET .../article response's last
    section ends with a deterministic attribution sentence built from the
    episode's own metadata -- never generated by the LLM, so it's always
    present regardless of what the pipeline produced."""
    episode, transcript = await _seed_episode_and_transcript(db_session)
    episode.channel_name = "The Real Channel"
    await db_session.commit()
    ...
    await ArticleRepository(db_session).replace(
        ...
        revision_count=0,
        sections=[
            ArticleSectionCandidate(sequence_number=0, heading="Intro", content="Some prose."),
            ArticleSectionCandidate(sequence_number=1, heading="Conclusion", content="Closing prose."),
        ],
    )
    await db_session.commit()
    ...
    # Never on a non-last section -- only the article's actual final section.
    assert sections[0]["content"] == "Some prose."
    last_content = sections[-1]["content"]
    assert last_content.startswith("Closing prose.")
    assert "This article is based on" in last_content
    assert "The Real Channel" in last_content
    assert episode.youtube_url in last_content
