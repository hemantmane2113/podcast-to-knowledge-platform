...
async def test_public_article_ends_with_deterministic_podcast_attribution(db_session: AsyncSession) -> None:
    """Regression test for the Live/Draft Article Workflow follow-up fix
    (requirement #4): the public response's last section ends with a
    deterministic attribution sentence built from the episode's own
    metadata -- never generated by the LLM."""
    episode, transcript = await _seed_episode_and_transcript(db_session, channel_name="The Real Channel")
    await _seed_article(db_session, episode, transcript, publish=True)
    ...
    assert response.status_code == 200
    sections = response.json()["sections"]
    # sequence_number 0 ("First") is never touched -- only the actual
    # last section (sequence_number 1, "Second") gets the attribution.
    assert sections[0]["content"] == "First section prose."
    last_content = sections[-1]["content"]
    assert last_content.startswith("Second section prose.")
