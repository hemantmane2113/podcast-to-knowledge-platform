...
def test_fixture_cleans_without_losing_or_duplicating_words() -> None:
    ...
    cleaned_word_count = sum(len(s.cleaned_text.split()) for s in segments)
...
def test_fixture_chunks_are_sequential_with_no_gaps() -> None:
    segments = build_transcript_segments()
    clean_transcript_segments(segments)
    chunks = chunk_transcript_segments(segments, _default_config())
...
def test_fixture_chunk_text_reconstructs_every_word_exactly_once() -> None:
    ...
    original_words = " ".join(s.cleaned_text for s in segments).split()
...
def test_fixture_long_uninterrupted_segment_is_traceable_and_intact() -> None:
    ...
    long_segment = next(s for s in segments if s.text.startswith("and honestly"))
    touching_chunks = [c for c in chunks if long_segment.id in c.source_segment_ids]
    assert len(touching_chunks) > 1  # confirms the split actually happened

    reconstructed = " ".join(c.text for c in touching_chunks)
    assert long_segment.cleaned_text in reconstructed
...
def test_fixture_chunking_is_deterministic() -> None:
    ...
    clean_transcript_segments(segments_a)
    segments_b = build_transcript_segments()
    clean_transcript_segments(segments_b)
    config = _default_config()

    chunks_a = chunk_transcript_segments(segments_a, config)
    chunks_b = chunk_transcript_segments(segments_b, config)
