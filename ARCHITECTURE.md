...
**Fallback** (`app/providers/llm/fallback_provider.py`): when `OPENAI_FALLBACK_ENABLED` is true and the primary isn't already `openai` (never `FallbackLLMProvider(openai, openai)`), the factory wraps the primary in `FallbackLLMProvider(primary, fallback)` — same `LLMProvider` interface, so `app/ai/nodes/` never knows fallback exists. On a *retryable* primary failure (`exc.retryable is True` — rate limit, 5xx, connection/timeout; see `LLMProviderRateLimitError`/`LLMProviderTransientError` below) it retries the same call once against OpenAI; a non-retryable failure (auth, missing config, a malformed/400 request — `LLMProviderAuthError`/`LLMProviderRequestError`) is never retried against the fallback, since switching providers wouldn't fix a bad credential or a bad request. Each concrete provider's SDK client is constructed with `max_retries=0` and a bounded per-request `timeout` (`app/providers/llm/_chat_completions.py`'s `DEFAULT_CLIENT_*` constants) — real incident: the Groq SDK's own retry-on-429 (honoring a `Retry-After` that can itself be 100s+, times the SDK's default 2 retries) previously consumed the entire arq job timeout before this fallback logic ever ran; application-level fallback is now the thing that switches providers quickly, not a long hidden SDK-level wait.
